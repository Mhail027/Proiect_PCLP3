...
pclass = df["Pclass"]
pc_options = set(pclass)
pclass_counters = np.full(len(pc_options) + 1, 0)
for elem in pclass:
...
fig, (surv_graphic, pc_graphic, sex_graphic) = plt.subplots(1, 3)

surv_graphic.pie(surv_counters, labels = surv_options, autopct = '%1.1f%%')
surv_graphic.set_title("Survived")
pc_graphic.pie(pclass_counters[1:4], labels = pc_options, autopct = '%1.1f%%')
pc_graphic.set_title("Pclass")
